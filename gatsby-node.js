    ...
    modules: project.modules.map(module => {
      ...
      if (module.type === 'media_collection')
        return _extends({}, module, {
          components: module.components.map(transformImage),
        })
      return module
    }),
  })
...
exports.sourceNodes = (() => {
  var _ref = _asyncToGenerator(function*(
    ...
    var _ref2 = yield axiosClient.get(
      `/users/${username}/projects?api_key=${apiKey}`
    )

    const projects = _ref2.data.projects

    var _ref3 = yield axiosClient.get(
      `/users/${username}/collections?api_key=${apiKey}`
    )

    const collections = _ref3.data.collections

    var _ref4 = yield axiosClient.get(`/users/${username}?api_key=${apiKey}`)
    ...
    const requests = projects.map(function(project) {
      return axiosClient.get(`/projects/${project.id}?api_key=${apiKey}`)
    })
